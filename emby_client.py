...
def _parse_tvdb_id(prov: dict) -> str | None:
    """Extract the numeric TVDB id (kept as a string) from an Emby ProviderIds
    dict. Real-world libraries sometimes carry junk in this field — e.g. an
    IMDb 'tt…' id pasted into the TVDB slot (issue #16) — and one bad value
    must not poison downstream int() indexing, so non-numeric values are
